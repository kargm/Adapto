...
(PUSH #P"/opt/ros/diamondback/stacks/ros_comm/clients/roslisp/asdf/"
...
(HANDLER-BIND ((STYLE-WARNING #'MUFFLE-WARNING) (WARNING #'PRINT))
  ...
  (SETF (SYMBOL-VALUE (INTERN "*CURRENT-ROS-PACKAGE*" :ROS-LOAD))
          "adapto_executive")
  (LET ((*STANDARD-OUTPUT* (MAKE-BROADCAST-STREAM)) (SYS "integration-test"))
  ...
  (LOAD
   (MERGE-PATHNAMES
    (MAKE-PATHNAME :NAME "adapto-integration-test-init.lisp" :DIRECTORY
                   '(:RELATIVE "roslisp" "adapto_executive"))
    (FUNCALL (SYMBOL-FUNCTION (INTERN "ROS-HOME" :ROS-LOAD))))
   :IF-DOES-NOT-EXIST NIL)
  (FUNCALL
   (SYMBOL-FUNCTION
    (READ-FROM-STRING "adapto-executive-tests:integration-tests-main")))
  (QUIT))